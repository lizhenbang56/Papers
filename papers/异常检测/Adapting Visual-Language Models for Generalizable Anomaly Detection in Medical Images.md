...
在测试过程中，为了准确预测图像级别（AC）和像素级别（AS）的异常，我们的方法采用了一个两分支多级特征比较架构，包括零样本分支和少样本分支，如图2（c）所示。 零样本分支。测试图像xtest经过MVFA处理，生成多级适应特征。然后，将这些特征与文本特征Ftext进行比较。零样本AC和AS结果，表示为czero和Szero，通过四个级别的平均softmax得分计算而得，czero = 1 4 P4 l=1 max G (softmax(Fcls,lF T text)), Szero = 1 4 P4 l=1BI(softmax(Fseg,lF T text))。这里，BI(·)将异常地图重塑为√ G× √ G并使用双线性插值将其恢复到原始输入图像分辨率，其中G表示网格数。 少样本分支。Dmed中少数标记的正常图像的多级视觉特征有助于构建一个特征存储库G，从而促成特征比较。少样本AC和AS得分，表示为cfew和Sfew，是通过在每个级别上测试特征与存储库特征之间的最小距离，通过最近邻搜索过程得出的，cfew = 1 4 P4 l=1 max G (min m∈G Dist(Fcls,l, m)), Sfew = 1 4 P4 l=1BI(min m∈G Dist(Fseg,l, m))。这里，Dist(·, ·)表示余弦距离，计算方法为1−cosine(·, ·)。最终预测的AC和AS结果结合了两个分支的结果：cpred = β1czero + β2cfew，Spred = β1Szero + β2Sfew。这里，β1和β2是零样本分支和少样本分支的权重因子，分别默认设置为0.5。
